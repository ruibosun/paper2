...
new <- subway_data_monthly%>%group_by(Country)%>%summarise(Total=sum(as.numeric(Ridership),na.rm=T))%>%filter(Country=="Chile")

kable(head(subway_data_monthly,5), caption = "Subway City in Chile (First 5 rows)")
```
