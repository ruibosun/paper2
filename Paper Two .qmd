...
Therefore, it is crucial to reduce air pollution by introducing a stainable policy and method. According to LI and YIN @f and Topalovic et al. @g, subways are the most effective mode of transportation for reducing congestion and greenhouse emissions due to their speed, efficiency, safety, and environmental benefits. Furthermore, as noted by Chen and Whalley @g . The operation of subways can lead to improvements in air quality, reduced mortality rates resulting from air pollution, increased productivity, and reduced social costs.
...
In table one ,The information is limited to the urban area surrounding Valparaiso and does not cover other parts of Chile. Furthermore, the data set offers no details regarding the underlying variables that might be responsible for changes in ridership over time.
...
#| echo: false
#| label: Tab-two
#| fig-cap: "subway_date_construction_begins"
library(here)
library(dplyr)
...
#| echo: false
#| label: fig-four
#| fig-cap: "Number of Turbine Manufacturers by Country"
library(readxl)
...
In figure five, the PM10 number for each nation in each year is displayed on this graph. The scatter diagram for the PM10 Annual mean values by year in the pm10 dataset with two fitted regression lines. The PM10 data to be used for the visualisation is first specified in the code, after which the Year variable is mapped to the x-axis and the PM10 Annual mean, ug/m3 variable to the y-axis.
...
In table three, the inauguration and construction times of various cities, as well as the plan year and an unnamed variable X, are listed in table three. Each community's unnamed infrastructure project is the subject of the material. The infrastructure project's completion year is indicated by the opening date, while the development phase's initial start year is indicated by the beginning building date. The project's original approach was developed during the plan year.
...
#| fig-cap: "Subway_City"
library(knitr)
kable(head(new_subway_city,10), caption = "Table 3: Subway_City (First 5 rows)")
```
